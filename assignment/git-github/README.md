...
git commit -a -m "message" stages and commits modifications and deletions to files Git already tracks. It does not include new, untracked files.

git commit -m "message" commits only what is already staged, whether that is a new file, a modification, or a deletion.

The commands and observed output are recorded below after the exercise is run.

First, I modified this tracked README and created task1-untracked.txt, then ran:
...
Task 2: Cherry-pick
The exercise creates several commits on main, creates a separate branch with multiple commits, then cherry-picks one selected commit back into main.

The final log and verification output are recorded below.

On git-homework-cherry-pick, git log --oneline --decorate -4 identified the second branch commit as the selected commit:
...
This brought cherry-pick-selected.txt into main; the branch-only file was not brought over.
